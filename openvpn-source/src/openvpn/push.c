...
receive_auth_failed(struct context *c, const struct buffer *buffer)
{
    ...
    /* AWS SAML CRV1 in-session re-auth: avoid SIGUSR1, trigger TLS
     * renegotiation within the same connection so the CRV1 state_id
     * remains valid.  Only active when --auth-retry interact is set. */
    else if (authfail_extended
             && buf_string_match_head_str(&buf, "CRV1:")
             && BLEN(&buf)
             && auth_retry_get() == AR_INTERACT)
    {
        ...
        /* Do NOT register any signal — stay in same TLS session */
        return;
    }
